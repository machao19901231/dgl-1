...
def worker_func(worker_id, args, g, features, labels, train_mask, val_mask, test_mask,
    ...
    recv_train = None
    if worker_id == 0:
        recv_train = dgl.contrib.sampling.SamplerReceiver(ip=args.ip, port=args.port_0, num_sender=2)
    elif worker_id == 1:
        recv_train = dgl.contrib.sampling.SamplerReceiver(ip=args.ip, port=args.port_1, num_sender=2)
    else worker_id == 2:
        recv_train = dgl.contrib.sampling.SamplerReceiver(ip=args.ip, port=args.port_2, num_sender=2)
